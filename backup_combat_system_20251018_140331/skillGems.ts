...
// Skill calculation cache
interface SkillCache {
  modifiers: Map<string, { result: any; timestamp: number }>
  damageWithStats: Map<string, { result: number; timestamp: number }>
}

const skillCache: SkillCache = {
  modifiers: new Map(),
  damageWithStats: new Map()
}

const SKILL_CACHE_TTL = 5000 // 5 seconds cache

function generateSkillCacheKey(skill: SkillGem): string {
  const supportGemsKey = skill.supportGems
    .map(sg => `${sg.id}-${sg.level}-${sg.rarity}`)
    .sort()
    .join('|')
  return `${skill.id}-${skill.level}-${skill.rarity}-${supportGemsKey}`
}

function generateSkillWithStatsCacheKey(skill: SkillGem, player: any): string {
  const skillKey = generateSkillCacheKey(skill)
  const playerKey = `${player.level || 1}-${player.dps || 2}-${JSON.stringify(player.attributes || {})}-${player.calculatedStats?.damage || 0}`
  return `${skillKey}-${playerKey}`
}
...
// Memoized version of applyModifiersToSkill
function applyModifiersToSkillMemoized(skill: SkillGem): {
  damage: number
  manaCost: number
  cooldown: number
  area: number
  ...
  const cacheKey = generateSkillCacheKey(skill)
  const now = Date.now()
  
  // Check cache
  const cached = skillCache.modifiers.get(cacheKey)
  if (cached && (now - cached.timestamp) < SKILL_CACHE_TTL) {
    return cached.result
  }
  
  // Calculate and cache result
  const result = applyModifiersToSkillInternal(skill)
  skillCache.modifiers.set(cacheKey, { result, timestamp: now })
  
  // Clean old cache entries periodically
  if (skillCache.modifiers.size > 100) {
    for (const [key, value] of skillCache.modifiers.entries()) {
      if ((now - value.timestamp) > SKILL_CACHE_TTL) {
        skillCache.modifiers.delete(key)
      }
    }
  }
...
// Internal implementation of modifier application
function applyModifiersToSkillInternal(skill: SkillGem): {
  damage: number
  manaCost: number
  cooldown: number
  area: number
...
// Comprehensive modifier application system (now uses memoized version)
export function applyModifiersToSkill(skill: SkillGem): {
  ...
  return applyModifiersToSkillMemoized(skill)
}
...
// Memoized version of calculateSkillDamageWithStats
function calculateSkillDamageWithStatsMemoized(skill: SkillGem, player: any): number {
  const cacheKey = generateSkillWithStatsCacheKey(skill, player)
  const now = Date.now()
  
  // Check cache
  const cached = skillCache.damageWithStats.get(cacheKey)
  if (cached && (now - cached.timestamp) < SKILL_CACHE_TTL) {
    return cached.result
  }
  
  // Calculate and cache result
  const result = calculateSkillDamageWithStatsInternal(skill, player)
  skillCache.damageWithStats.set(cacheKey, { result, timestamp: now })
  
  // Clean old cache entries periodically
  if (skillCache.damageWithStats.size > 100) {
    for (const [key, value] of skillCache.damageWithStats.entries()) {
      if ((now - value.timestamp) > SKILL_CACHE_TTL) {
        skillCache.damageWithStats.delete(key)
      }
    }
  }
...
// Internal implementation of skill damage calculation with stats
function calculateSkillDamageWithStatsInternal(skill: SkillGem, player: any): number {
...
// Comprehensive skill damage calculation that integrates character stats (now uses memoized version)
export function calculateSkillDamageWithStats(skill: SkillGem, player: any): number {
  return calculateSkillDamageWithStatsMemoized(skill, player)
}
